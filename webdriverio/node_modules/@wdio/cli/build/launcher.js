...
class Launcher {
  constructor(configFile, argv) {
    ...
    _logger.default.setLogLevelsConfig(config.logLevels);
    ...
    this.interface = new _interface.default(config, specs, totalWorkerCnt, this.runner.stdout, this.runner.stderr);
    ...
    for (let capabilities of caps) {
      this.schedule.push({
        ...
        specs: this.configParser.getSpecs(capabilities.specs, capabilities.exclude).map(s => ({
          files: [s],
          retries: config.specFileRetries
        })),
  ...
  startInstance(specs, caps, cid, server, rid, retries) {
    ...
    let config = this.configParser.getConfig(); // Retried tests receive the cid of the failing test as rid
