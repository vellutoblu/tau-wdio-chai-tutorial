...
function n(n3, i, o2, f2) {
  let s2, u, l = r(n3, o2), c = function(e2) {
  ...
  if (void 0 === a) {
    let e2, n4, r2, t2;
    for (t2 in i)
      n4 && t2.length < n4.length || ("/" === t2[t2.length - 1] && l.startsWith(t2) ? (u = l.substring(t2.length), n4 = t2) : t2.length > 1 && (r2 = t2.indexOf("*", 1), ~r2 && (e2 = RegExp("^" + t2.substring(0, r2) + "(.*)" + t2.substring(1 + r2)).exec(l), e2 && e2[1] && (u = e2[1], n4 = t2))));
    a = i[n4];
  }
  return a || e(n3, l), s2 = t(a, c), s2 || e(n3, l, 1), u && function(e2, n4) {
    let r2, t2 = 0, i2 = e2.length, o3 = /[*]/g, f3 = /[/]$/;
    for (; t2 < i2; t2++)
      e2[t2] = o3.test(r2 = e2[t2]) ? r2.replace(o3, n4) : f3.test(r2) ? r2 + n4 : r2;
  }(s2, u), s2;
}
function r(e2, n3, r2) {
  if (e2 === n3 || "." === n3)
    return ".";
  let t2 = e2 + "/", i = t2.length, o2 = n3.slice(0, i) === t2, f2 = o2 ? n3.slice(i) : n3;
...
function s(e2, n3, t2) {
  return "#" === (n3 = r(e2.name, n3 || "."))[0] ? f(e2, n3, t2) : o(e2, n3, t2);
}
...
function inject(options) {
  ...
  if (_path.sep !== "/") {
    modulesMap.forEach((mod, key) => {
      modulesMap.set(
        key,
        Array.isArray(mod) ? [mod[0].split(_path.sep).join("/"), mod[1]] : mod.split(_path.sep).join("/")
      );
    });
  }
  const firstpass = new RegExp(
    `(?:${Array.from(modulesMap.keys()).concat(Array.from(expressionsMap.keys())).map(escape).join("|")})`,
    "g"
  );
  return {
    name: "inject",
...
async function polyfillPath(mod) {
  ...
  const jspmPath = _path.resolve.call(void 0, 
    _chunkJWDSS33Ocjs.__require.resolve(`@jspm/core/nodelibs/${mod}`),
    // ensure "fs/promises" is resolved properly
    "../../.." + (mod.includes("/") ? "/.." : "")
  );
  const jspmPackageJson = await _localpkg.loadPackageJSON.call(void 0, jspmPath);
  const exportPath = s(jspmPackageJson, `./nodelibs/${mod}`, {
    browser: true
  });
  const exportFullPath = _localpkg.resolveModule.call(void 0, _path.join.call(void 0, jspmPath, _optionalChain([exportPath, 'optionalAccess', _2 => _2[0]]) || ""));
...
async function inject2(content, options, id) {
  ...
  const parse4 = (input, opts) => parse3(input, { ...opts, sourceType: "module", ecmaVersion: "latest" });
  const result = injectPlugin.transform.call(
    { warn: console.warn, parse: parse4 },
    content,
    _nullishCoalesce(id, () => ( ""))
  );
  return _optionalChain([result, 'optionalAccess', _3 => _3.code]) || content;
}
async function polyfillGlobals(content, globals = {}, mods = {}) {
  return inject2(
    ...
    {
      expressions: {
        __filename: _optionalChain([globals, 'optionalAccess', _4 => _4.__filename]) || "/",
        __dirname: _optionalChain([globals, 'optionalAccess', _5 => _5.__dirname]) || "/"
      },
      modules: removeEmpty({
